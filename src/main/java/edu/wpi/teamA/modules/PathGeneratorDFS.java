...
public class PathGeneratorDFS<T extends Node> implements PathGenerator<T> {
  ...
  public ArrayList<T> findPath(T from, T to, int elev) {
    ...
    Stack<PathingNode<T>> nextNodes = new Stack<>();
    ...
    while (!nextNodes.isEmpty()) {
      ...
      PathingNode<T> nextNode = nextNodes.pop();
          ...
          .forEach(
              connection -> {
                ...
                if (neighborNode == null) {
                  if ((elev != 1 || !nextNode.getCurrent().getType().equals("STAI"))
